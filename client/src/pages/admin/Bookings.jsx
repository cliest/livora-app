...
export default function AdminBookings() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.listBookings({ ...(status ? { status } : {}), page }),
  });
      ...
      <div className="bg-white border border-line rounded-[18px] overflow-x-auto">
        <table className="w-full text-[0.88rem]">
          ...
          <tbody>
            {isLoading && (
              <tr>
                <td className="px-4 py-4 text-muted" colSpan={7}>Loading…</td>
              </tr>
            )}
            {!isLoading && data?.items?.length === 0 && (
              <tr>
                <td className="px-4 py-4 text-muted" colSpan={7}>No bookings found.</td>
              </tr>
            )}
            {data?.items?.map((b) => (
